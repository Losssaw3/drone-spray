...
|НС-11| При компрометации модуля GPS злоумышленник подменил реальные координаты дрона, в результате чего дрон вылетел из авторизованного района (Нарушено ЦБ 1)|
|НС-12| При компрометации модуля GPS злоумышленник подменил реальные координаты дрона, в результате чего распыление смеси началось вне района работ (Нарушено ЦБ 3)|
|НС-13| При компрометации модуля GPS злоумышленник подменил реальные координаты дрона, в результате чего УЦ получил недостоверную информацию о местоположении дрона (Нарушено ЦБ 4)|
...
*Так как ИНС и GPS выполняют смежную функцию, но при стандартном выполнении миссии система берет за основу данные от GPS, то компрометация ИНС не нарушает ЦБ при стандартной работе системе, но функционал ИНС является критическим его компрометация может нарушит ЦБ  
...
|9. Контроль состояния дрона|Отвечает за сбор данных о дроне (местоположение, высота, состояние аккамулятора)||
...
|15. ИНС|Ведет GPS-независимую карту полета. Строит аварийный маршрут возвращения на базу (в переработанной архитектуре основной модулб получения текущих координат)||
|16. Комплексирование|Сравнивает координаты полученные от GPS и ИНС и производит их комплексирование||
...
![alt text](images/NewScenario.png)
...
|7. GPS|$\textcolor{red}{\textsf{Недоверенный}}$|При компрометации, модуль комплексировнания сможет определить сбой в модуле GPS||SS|Аппаратный модуль спутниковой системы связи|
...
|15. ИНС|$\textcolor{green}{\textsf{Доверенный}}$|Является опорным модулем для получения координат системемой, при компрометации нарушает ЦБ 1,3,4|Модуль необхоодим что отработки внештатных ситуации при компрометации GPS, который является доверенным по ПБ|SM|Союственная система отсчета|
|16. Комплексирование|$\textcolor{green}{\textsf{Доверенный}}$|При компрометации нарушает ЦБ 1, 3, 4|Модуль проводит комплексирование координат полученных из модулей ИНС и GPS|SS|Сравнение координат и выдача приближенного значения близкого к реальному|
