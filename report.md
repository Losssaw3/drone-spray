...
*Так как ИНС и GPS выполняют смежную функцию, но при стандартном выполнении миссии система берет за основу данные от GPS, то компрометация ИНС не нарушает ЦБ при стандартной работе системе, но функционал ИНС является критическим
...
|15. ИНС|Ведет GPS-независимую карту полета. Строит аварийный маршрут возвращения на базу||
...
|15. ИНС|$\textcolor{green}{\textsf{Доверенный}}$|При компрометации не нарушает ЦБ|Модуль необхоодим что отработки внештатных ситуации при компрометации GPS, который является доверенным по ПБ|SM|Союственная система отсчета|
